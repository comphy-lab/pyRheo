...
from pyRheo.creep_model import CreepModel  # Import your updated CreepModel class
import io
import sys
...
class CreepApp:
    def __init__(self, root):
        self.root = root
        self.root.title("pyRheo Creep Model Fitting")
    ...
    def load_data(self):
        ...
        if file_path:
            try:
                ...
                self.J_creep = data['Creep Compliance'].values
                messagebox.showinfo("Data Load", "Data loaded successfully.")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to load data: {e}")

    def run_model(self):
        if self.time_data is None or self.J_creep is None:
    ...
    def plot_results(self):
        ...
        ax.scatter(self.time_data, self.J_creep, label="Data", color="dodgerblue")
        predicted_J_creep = self.model.predict(self.time_data)
        ax.plot(self.time_data, predicted_J_creep, label="Fitted Model", color="orangered")

        ax.set_xscale('log')
        ax.set_yscale('log')
        ax.set_xlabel("time", fontsize=14)
        ax.set_ylabel("creep compliance", fontsize=14)
    ...
    def download_results(self):
        ...
        results_df = pd.DataFrame({
            "Time": self.time_data,
            "Creep Compliance": predicted_J_creep
        })
