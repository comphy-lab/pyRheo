...
from pyRheo.rotation_model import RotationModel  # Import your updated RotationModel class
import io
import sys
...
class CreepApp:
    def __init__(self, root):
        self.root = root
        self.root.title("pyRheo Rotation Model Fitting")
        ...
        self.model_var.set("PowerLaw")  # Default model
        self.models = [
    ...
    def load_data(self):
        ...
        if file_path:
            try:
                ...
                self.gamma_dot = data['Shear Rate'].values
                self.eta = data['Viscosity'].values
                messagebox.showinfo("Data Load", "Data loaded successfully.")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to load data: {e}")

    def run_model(self):
        if self.gamma_dot is None or self.eta is None:
        ...
        try:
            ...
            self.model.fit(self.gamma_dot, self.eta)
    ...
    def plot_results(self):
        ...
        ax.scatter(self.gamma_dot, self.eta, label="Data", color="dodgerblue")
        predicted_eta = self.model.predict(self.gamma_dot)
        ax.plot(self.gamma_dot, predicted_eta, label="Fitted Model", color="orangered")
    ...
    def download_results(self):
        ...
        results_df = pd.DataFrame({
            "Shear Rate": self.gamma_dot,
            "Viscosity": predicted_eta
        })
