...
from pyRheo.relaxation_model import RelaxationModel  # Import your updated RelaxationModel class
import io
import sys
...
class RelaxationApp:
    def __init__(self, root):
        self.root = root
        self.root.title("pyRheo Relaxation Model Fitting")
    ...
    def load_data(self):
        ...
        if file_path:
            try:
                ...
                self.G_relax = data['Relaxation Modulus'].values
                messagebox.showinfo("Data Load", "Data loaded successfully.")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to load data: {e}")

    def run_model(self):
        if self.time_data is None or self.G_relax is None:
        ...
        try:
            ...
            self.model.fit(self.time_data, self.G_relax)
    ...
    def plot_results(self):
        ...
        ax.scatter(self.time_data, self.G_relax, label="Data", color="dodgerblue")
        predicted_G_relax = self.model.predict(self.time_data)
        ax.plot(self.time_data, predicted_G_relax, label="Fitted Model", color="orangered")

        ax.set_xscale('log')
        ax.set_yscale('log')
        ax.set_xlabel("time", fontsize=14)
        ax.set_ylabel("relaxation modulus", fontsize=14)
    ...
    def download_results(self):
        ...
        results_df = pd.DataFrame({
            "Time": self.time_data,
            "Relaxation Modulus": predicted_G_relax
        })
