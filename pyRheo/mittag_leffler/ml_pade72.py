...
# Pade for Mittag-Leffler m=7, n=2 according to I. O. Sarumi, K. M. Furati and A. Q. M. Khaliq, Journal of Scientific Computing, 2020, 82, 1–27.

# Pade for Mittag-Leffler m=7, n=2
def system_72ab(alpha, beta):
    A = np.array([
        ...
        [0, 0, 0, gamma(beta - alpha) / gamma(beta + 5 * alpha), -gamma(beta - alpha) / gamma(beta + 4 * alpha), gamma(beta - alpha) / gamma(beta + 3 * alpha), -gamma(beta - alpha) / gamma(beta + 2 * alpha)],
        [0, 0, 1, 0, 0, 0, -1]
    ])
    
    b = np.array([0, 0, 0, -1, gamma(beta - alpha) / gamma(beta), -gamma(beta - alpha) / gamma(beta + alpha), -gamma(beta - alpha) / gamma(beta - 2 * alpha)])
    
    return np.linalg.solve(A, b)

def system_72aa(alpha):
    A = np.array([
        ...
        [0, 0, gamma(-alpha) / gamma(5 * alpha), -gamma(-alpha) / gamma(4 * alpha), gamma(-alpha) / gamma(3 * alpha), -gamma(-alpha) / gamma(2 * alpha)],
        [0, 1, 0, 0, 0, -1]
    ])
...
def R_alpha_beta_7_2(x, alpha, beta):
    if beta > alpha:
        coefficients = system_72ab(alpha, beta)
        p = coefficients[:3]
        q = coefficients[3:]
        R_val = (1 / gamma(beta - alpha)) * (p[0] + p[1] * (-x) + p[2] * (-x)**2 + (-x)**3) / (q[0] + q[1] * (-x) + q[2] * (-x)**2 + q[3] * (-x)**3 + x**4)
    else:
        coefficients = system_72aa(alpha)
