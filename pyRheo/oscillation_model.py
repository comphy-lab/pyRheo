...
class OscillationModel(BaseModel):
    def __init__(self, model="Maxwell", method="RSS", initial_guesses="manual", bounds="auto", minimization_algorithm="Nelder-Mead", num_initial_guesses=64):
        ...
        if model == "auto":
            ...
            self.pca_prime = joblib.load('./pca_model_prime.joblib')
            self.pca_double_prime = joblib.load('./pca_model_double_prime.joblib')
            self.classifier = joblib.load('./mlp_model_oscillation.joblib')
