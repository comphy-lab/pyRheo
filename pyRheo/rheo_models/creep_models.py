...
def FractionalMaxwellGel(G_s, V, alpha, t, errorInserted=0):
    """
...
def FractionalZenerLiquidS(G_p, G, eta_s, beta, t, errorInserted=0):
    """
...
def FractionalZenerLiquidD(eta_p, G, eta_s, beta, t, errorInserted=0):
    """
...
def FractionalZenerS(G_p, G, V, alpha, beta, t, errorInserted=0):
    """
    ...
    ndarray
        The creep compliance response at each time point in `t`.
    """
    def J_hat(s, G_p, G, V, alpha, beta):
        # Calculate terms for the numerator of the Laplace transform
        term1 = np.divide(1, s)
        ...
        denominator2 = np.multiply(G_p, numerator)
    ...
    def inverse_laplace_transform(J_hat, t):
        # Use mpmath's invertlaplace function for the inverse Laplace transform
        return mpmath.invertlaplace(lambda p: J_hat(p, G_p, G, V, alpha, beta), t, method='talbot')
